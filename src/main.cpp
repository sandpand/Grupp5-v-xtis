...
public:
  ...
  void setNeopixelColor(int red, int green, int blue)
  {
    ...
    {                                                        // Loop through each NeoPixel LED
 ...
 * @brief A class to manage the smart garden system, including sensors, water pump, LEDs, and display.
 */

/**
 * @brief Initializes the smart garden system components.
 *
 * This function sets up the pin modes for sensors, water pump, and LEDs,
 * initializes the LED controller, LCD display, and serial communication.
 */
void begin();

/**
 * @brief Updates the smart garden system by reading sensor values and controlling components.
 *
 * This function reads the soil moisture and light intensity values, displays them on the LCD,
 * prints them to the Serial monitor, and controls the water pump, LEDs, and grow light based on the readings.
 */
void update();

/**
 * @brief Controls the water pump and RGB LEDs based on soil moisture value.
 *
 * @param soilValue The soil moisture value read from the soil sensor.
 *
 * - If soilValue > 950: Turns on the water pump at 80% speed and sets LED color to Blue.
 * - If 450 <= soilValue <= 950: Turns on the water pump at 50% speed and sets LED color to Red.
 * - If soilValue < 450: Turns off the water pump and sets LED color to Green.
 */
void controlWaterPumpAndLEDs(int soilValue);

/**
 * @brief Controls the grow light based on light intensity value.
 *
 * @param lightValue The light intensity value read from the light sensor.
 *
 * - If lightValue < 20: Turns on the grow light with a warm orange color.
 * - If lightValue >= 20: Turns off the grow light.
 */
...
public:
  ...
  void update()
  {
    ...
    Serial.print("Soil: ");
    Serial.println(soilVal); // Display soil value on Serial monitor
    Serial.print("Light: ");
    Serial.println(lightVal); // Display light value on Serial monitor
